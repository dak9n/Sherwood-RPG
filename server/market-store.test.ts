...
test('истёкшие лоты не считаются против лимита при новом выставлении', () => {
  const m = fresh();
  for (let i = 0; i < MAX_LOTS_PER_SELLER; i++) m.list('ann', 'Ann', sword(), 100, T0);
  // Все лоты истекли, и с тех пор не было ни browse, ни mine — только list.
  assert.ok(m.list('ann', 'Ann', sword(), 100, T0 + LISTING_TTL + 1).ok, 'истёкшие освободили лимит');
});
...
test('купить: лот исчезает, продавцу капает выручка минус комиссия, есть история', () => {
  ...
  // покупателю — сам предмет ПОЧТОЙ (надёжно: не потеряется при полной сумке/потере ответа)
  const bobMail = m.mailFor('bob');
  assert.equal(bobMail.length, 1);
  assert.equal(bobMail[0].kind, 'item');
  if (bobMail[0].kind === 'item') assert.deepEqual(bobMail[0].item, { id: 'sword', qty: 1, sharpen: 3 }, 'заточенный меч пришёл покупателю');
  // история у обоих
