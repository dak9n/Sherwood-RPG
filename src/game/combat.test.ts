...
import { hitRect, distSq, rollDamage, fireballDamage, arrowRainDamage, playerDamageTaken } from './combat.ts';
...
test('барьер режет входящий урон на 80% поверх брони', () => {
  assert.equal(playerDamageTaken(20, 0, false), 20, 'без брони и щита — как прилетело');
  assert.equal(playerDamageTaken(20, 5, false), 15, 'броня вычитается');
  assert.equal(playerDamageTaken(20, 0, true), 4, 'щит оставляет пятую часть');
  assert.equal(playerDamageTaken(20, 5, true), 3, 'щит режет уже пробитое бронёй (15 → 3)');
});

test('урон по герою никогда не падает ниже единицы', () => {
  assert.equal(playerDamageTaken(3, 100, false), 1, 'броня не даёт полной неуязвимости');
  assert.equal(playerDamageTaken(3, 0, true), 1, 'щит тоже не обнуляет урон');
  assert.equal(playerDamageTaken(1, 50, true), 1, 'броня и щит вместе — всё равно 1');
});

test('урон волны града стрел растёт с уровнем и не уходит в минус', () => {
  assert.equal(arrowRainDamage(1), 4, 'на первом уровне — база за волну');
  assert.equal(arrowRainDamage(2), 6, '+2 за уровень');
  assert.equal(arrowRainDamage(5), 12);
  assert.equal(arrowRainDamage(0), 4, 'уровень ниже 1 — как первый');
  assert.equal(arrowRainDamage(-3), 4);
  assert.equal(arrowRainDamage(3.9), 8, 'дробный уровень — вниз до целого');
});
