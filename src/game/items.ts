...
export const ITEMS: Record<string, ItemDef> = {
  ...
  // --- Шлемы helm1..helm6 ---
  //
  // Шесть шлемов, у каждого свой РИСОВАННЫЙ вид на все четыре стороны
  // (worn/helmN.png, собран из четырёх ракурсов armor-icons/helmN через
  // tools/worn-from-views.py). Спрайт цепляется к предмету по id: игра грузит
  // worn/<id>.png для надетого шлема. Поэтому tint им не нужен — расцветку
  // задаёт сам рисунок, а не перекраска волос.
  //
  // Иконка в сумке — из того же вида «спереди», допечатана в атлас брони:
  // aico(102)..aico(107) (11-й ряд, дописанный нами). Имена и статы пока
  // заглушки — заказчик переименует и добавит ещё шлемов позже.
  helm1: {
    id: 'helm1', name: 'Helmet 1', tab: 'armor',
    icon: aico(102), stack: 1, slot: 'helm', bonus: { def: 1 }, rarity: 'common',
  },
  helm2: {
    id: 'helm2', name: 'Helmet 2', tab: 'armor',
    icon: aico(103), stack: 1, slot: 'helm', bonus: { def: 2 }, rarity: 'uncommon',
  },
  helm3: {
    id: 'helm3', name: 'Helmet 3', tab: 'armor',
    icon: aico(104), stack: 1, slot: 'helm', bonus: { def: 2, hp: 10 }, rarity: 'rare',
  },
  helm4: {
    id: 'helm4', name: 'Helmet 4', tab: 'armor',
    icon: aico(105), stack: 1, slot: 'helm', bonus: { def: 3, hp: 10 }, rarity: 'rare',
  },
  helm5: {
    id: 'helm5', name: 'Helmet 5', tab: 'armor',
    icon: aico(106), stack: 1, slot: 'helm', bonus: { def: 3, hp: 15, mp: 10 }, rarity: 'epic',
  },
  helm6: {
